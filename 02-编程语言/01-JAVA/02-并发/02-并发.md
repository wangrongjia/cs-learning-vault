...
##  示例1：线程不安全的 Controller 成员变量

```java
@RestController
public class OrderController {

    // ❌ 线程不安全：这个 SimpleDateFormat 是所有线程共享的！
...
```
###  问题原因：
- Spring Boot 默认是 **单例 Controller**（所有请求共用一个实例）。
- `SimpleDateFormat` 是 **非线程安全类**。
- 多个请求并发调用 `formatDate()` 时会：
    - 报 `NumberFormatException`；
    - 返回错误日期；
    - 甚至出现死循环（老版本 JDK）。

###  模拟现象：

当高并发请求 `/format` 时，日志可能出现：
```
Exception in thread "http-nio-8080-exec-12" java.lang.NumberFormatException: For input string: ""
```
或者返回一些莫名其妙的时间字符串。

###  正确写法 1：方法内创建局部变量（线程隔离）

```java
@GetMapping("/format-safe")
public String formatDateSafe() {
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    return sdf.format(new Date());
}
```

> 每次请求创建自己的 `SimpleDateFormat`，不同线程互不干扰。

### 正确写法 2：使用线程安全的 `DateTimeFormatter`

```java
private static final DateTimeFormatter FORMATTER =
        DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");

@GetMapping("/format-safe2")
public String formatDateSafe2() {
    return LocalDateTime.now().format(FORMATTER);
}
```

> `DateTimeFormatter` 是 **线程安全的**，推荐使用。
...
public class UserCacheService {
    ...
    // ❌ 非线程安全：HashMap 在多线程环境中会出问题！
    private final Map<String, String> userCache = new HashMap<>();

    public void putUser(String userId, String name) {
        userCache.put(userId, name);  // 并发写入
    }

    public String getUser(String userId) {
        return userCache.get(userId); // 并发读
    }
}
```
...
public class UserController {
    ...
    @PostMapping("/cache")
    public String cacheUser(@RequestParam String id, @RequestParam String name) {
        userCacheService.putUser(id, name);
        return "OK";
    }
...
`@Service` 默认是 **Spring 单例 Bean**，意味着：

- 整个应用只有一个 `UserCacheService` 实例；
- 这个实例的 `userCache`（HashMap）被所有请求线程共享；
- 多个请求同时 `put` / `get` 时可能出错。
...
### 正确写法 1：使用线程安全的集合（ConcurrentHashMap）
...
`ConcurrentHashMap` 内部做了分段锁或 CAS 控制，支持高并发访问。


###  正确写法 2：使用本地缓存框架（推荐）

如果你要的是**内存级缓存**，更好的做法是用成熟的缓存组件：
...
Caffeine 是 Spring 官方推荐的高性能缓存库（Spring Boot 2.6+ 可直接使用）。
### 正确写法 3：放入 Redis 这种共享存储

如果多实例部署（微服务架构），本地缓存会失效，应使用分布式缓存：
...
> **Java 中线程不安全的根本原因，是多个线程同时访问同一个可变状态（mutable state），而没有恰当的同步机制。**

**上面的两个例子  都是 一个单例的类中的变量被并发访问了**
这种情况是最典型的线程不安全来源：
**但注意：线程不安全不止这种情况**

虽然这是最常见的一类，但**并不是唯一的情况**。  
更一般地说，线程不安全还可能出现在以下几种情形👇：
...
静态变量在整个 JVM 里只有一份实例，被所有线程共享访问。  
如果多线程修改，没有同步，也会出问题。

### 2️⃣ **多线程共享了同一个对象实例**

即使类本身不是单例，只要你把同一个对象传给多个线程，也不安全：

```java
SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");

new Thread(() -> sdf.format(new Date())).start();
new Thread(() -> sdf.format(new Date())).start();
```

> `SimpleDateFormat` 本身就不是线程安全的，  
> 不论它在哪个类中定义，只要被多个线程同时使用，就可能出错。

### 3️⃣ **方法里使用了共享对象**

即使方法看起来是局部的，但操作的对象来自外部，也会不安全：
方法相互调用，链路很深，如果变量相互传递，很容易线程不安全

```java
public void process(List<String> sharedList) {
    sharedList.add("data"); // 如果这个 List 被多个线程同时使用，就不安全
}
```
...
- `DecimalFormat`  
    这些类**默认都不是线程安全的**。  
    无论它们放在单例类、静态字段，还是被共享传入，都可能产生并发问题。

###  安全的几种方式（总结）

|场景|解决思路|
|---|---|
|单例 Bean 中定义可变字段|使用局部变量 / 加锁 / 使用并发安全类|
|静态变量共享|同上|
|非线程安全类（如 SimpleDateFormat）|每次创建新实例 / ThreadLocal / 使用线程安全替代类|
|集合|用 `ConcurrentHashMap`、`CopyOnWriteArrayList`、`Collections.synchronizedList` 等|
|复杂共享状态|使用 synchronized、Lock、Atomic 系列原子类|


> **线程不安全 ≠ 单例才会有问题。**  
> 任何时候，只要**多个线程能同时访问同一个可变对象且有写操作**，而又**没同步机制**，就可能线程不安全。
